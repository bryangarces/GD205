...
public class BoardMovement : MonoBehaviour
{
    ...
    void Start()
    {
        //assign the playerPos to whatever the position is of this gameobjects transform position
    ...
    void Update()
    {
        ...
        for (int i = 0; i < hazards.Length; i++)
        {
                    ...
                    playerPos.z == hazards[i].transform.position.z)
            {
                ...
                playerMessage.text = "Green is Win";
            }
        }
        ...
        {
            //if the players x and z position are the same as the current hazard we are checking...
            if (playerPos.x == destination[i].transform.position.x &&
                    playerPos.z == destination[i].transform.position.z)
            {
                //then reset the player to the start position
                playerPos += transform.up;
